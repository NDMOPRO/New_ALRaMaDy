    ...
    <div className="h-screen flex bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100" dir="rtl">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {showLog && (
          ...
          {/* === SECTION 6: Column Ops === */}
          {activeSection === "colOps" && (
            <SectionCard title="عمليات الأعمدة" icon="view_column" defaultOpen>
              {!activeTable ? <p className="text-gray-400">يرجى تحميل بيانات أولاً</p> : (() => {
                const t = activeTable; const cols = t.columns.map((c) => c.name);
                const runOp = () => {
                  if (!colOpCol) { setColOpResult("اختر عمود"); return; }
                  if (colOpType === "rename" && colOpParam) { updateTable(t.id, (p) => ({ ...p, columns: p.columns.map((c) => c.name === colOpCol ? { ...c, name: colOpParam } : c), rows: p.rows.map((r) => { const nr = { ...r }; nr[colOpParam] = nr[colOpCol]; delete nr[colOpCol]; return nr; }) })); setColOpResult(`تم: ${colOpCol} -> ${colOpParam}`); }
                  else if (colOpType === "split") { const sep = colOpParam || " "; updateTable(t.id, (p) => { const mx = Math.max(...p.rows.map((r) => String(r[colOpCol] ?? "").split(sep).length)); const nc = Array.from({ length: mx }, (_, i) => `${colOpCol}_${i + 1}`); return { ...p, columns: [...p.columns, ...nc.map((n) => ({ name: n, dtype: "string" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: n, sampleValues: [] }))], rows: p.rows.map((r) => { const parts = String(r[colOpCol] ?? "").split(sep); const nr = { ...r }; nc.forEach((n, i) => (nr[n] = parts[i] || null)); return nr; }) }; }); setColOpResult(`تم تقسيم ${colOpCol}`); }
                  else if (colOpType === "merge" && colOpParam) { const nn = `${colOpCol}_${colOpParam}`; updateTable(t.id, (p) => ({ ...p, columns: [...p.columns, { name: nn, dtype: "string" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: nn, sampleValues: [] }], rows: p.rows.map((r) => ({ ...r, [nn]: `${r[colOpCol] ?? ""} ${r[colOpParam] ?? ""}` })) })); setColOpResult(`تم دمج -> ${nn}`); }
                  else if (colOpType === "clean") { updateTable(t.id, (p) => ({ ...p, rows: p.rows.map((r) => ({ ...r, [colOpCol]: typeof r[colOpCol] === "string" ? r[colOpCol].trim().replace(/\s+/g, " ") : r[colOpCol] })) })); setColOpResult(`تم تنظيف ${colOpCol}`); }
                  else if (colOpType === "derive") { const ex = colOpParam || `[${colOpCol}] * 2`; const nn = colOpParam2 || `${colOpCol}_مشتق`; updateTable(t.id, (p) => ({ ...p, columns: [...p.columns, { name: nn, dtype: "float" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: nn, sampleValues: [] }], rows: p.rows.map((r) => { let v: CellValue; try { v = Function("row", `"use strict"; return (${ex.replace(/\[([^\]]+)\]/g, (_: string, c: string) => `row["${c}"]`)})`)({ ...r }); } catch { v = "#ERR"; } return { ...r, [nn]: v }; }) })); setColOpResult(`عمود مشتق: ${nn}`); }
                  addLog(`عملية عمود: ${colOpType} on ${colOpCol}`);
                };
                return (
                  <div className="space-y-4">
                    <div className="flex gap-2 flex-wrap">{(["rename", "split", "merge", "clean", "derive"] as const).map((op) => (<button key={op} onClick={() => setColOpType(op)} className={`px-3 py-1.5 rounded-lg text-sm ${colOpType === op ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800"}`}>{{ rename: "تسمية", split: "تقسيم", merge: "دمج", clean: "تنظيف", derive: "مشتق" }[op]}</button>))}</div>
                    <div className="flex gap-2 flex-wrap items-end">
                      <div><label className="block text-xs text-gray-500 mb-1">العمود</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={colOpCol} onChange={(e) => setColOpCol(e.target.value)}><option value="">اختر</option>{cols.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
                      {(colOpType === "rename" || colOpType === "split" || colOpType === "derive") && <div><label className="block text-xs text-gray-500 mb-1">{colOpType === "derive" ? "التعبير" : "معامل"}</label><input className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 w-48" value={colOpParam} onChange={(e) => setColOpParam(e.target.value)} /></div>}
                      {colOpType === "merge" && <div><label className="block text-xs text-gray-500 mb-1">العمود الثاني</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={colOpParam} onChange={(e) => setColOpParam(e.target.value)}><option value="">اختر</option>{cols.filter((c) => c !== colOpCol).map((c) => <option key={c} value={c}>{c}</option>)}</select></div>}
                      {colOpType === "derive" && <div><label className="block text-xs text-gray-500 mb-1">اسم جديد</label><input className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={colOpParam2} onChange={(e) => setColOpParam2(e.target.value)} /></div>}
                      <button className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700" onClick={runOp}><MIcon name="play_arrow" className="text-lg align-middle ml-1" /> تنفيذ</button>
                    </div>
                    {colOpResult && <div className="p-3 rounded-lg bg-green-50 text-green-700 text-sm">{colOpResult}</div>}
                    <DataTable data={activeTable} maxRows={8} />
                  </div>
                );
              })()}
            </SectionCard>
          ...
          {/* === SECTION 7: Table Ops === */}
          {activeSection === "tableOps" && (
            <SectionCard title="عمليات الجداول" icon="table_chart" defaultOpen>
              {!activeTable ? <p className="text-gray-400">يرجى تحميل بيانات أولاً</p> : (() => {
                const t = activeTable; const cols = t.columns.map((c) => c.name); const others = tables.filter((tb) => tb.id !== t.id);
                const run = () => {
                  if (tableOpType === "group" && tableOpGroupCol && tableOpAggCol) {
                    const groups = new Map<string, Record<string, CellValue>[]>(); for (const r of t.rows) { const k = String(r[tableOpGroupCol] ?? ""); if (!groups.has(k)) groups.set(k, []); groups.get(k)!.push(r); }
                    const rows: Record<string, CellValue>[] = []; groups.forEach((rws, key) => { const vs = rws.map((r) => typeof r[tableOpAggCol] === "number" ? r[tableOpAggCol] as number : 0); let v = 0;
                      if (tableOpAgg === "sum") v = vs.reduce((a, b) => a + b, 0); else if (tableOpAgg === "avg") v = vs.length ? vs.reduce((a, b) => a + b, 0) / vs.length : 0; else if (tableOpAgg === "count") v = rws.length; else if (tableOpAgg === "min") v = Math.min(...vs); else v = Math.max(...vs);
                      rows.push({ [tableOpGroupCol]: key, [`${tableOpAgg}_${tableOpAggCol}`]: Math.round(v * 100) / 100, عدد: rws.length }); });
                    const res: TableData = { id: crypto.randomUUID(), name: `${t.name}_grouped`, columns: [{ name: tableOpGroupCol, dtype: "string", nullRatio: 0, uniqueCount: rows.length, semanticLabel: tableOpGroupCol, sampleValues: [] }, { name: `${tableOpAgg}_${tableOpAggCol}`, dtype: "float", nullRatio: 0, uniqueCount: 0, semanticLabel: "measure", sampleValues: [] }, { name: "عدد", dtype: "int", nullRatio: 0, uniqueCount: 0, semanticLabel: "count", sampleValues: [] }], rows };
                    setTableOpResult(res); addTable(res); addLog(`Group: ${tableOpGroupCol} ${tableOpAgg}(${tableOpAggCol})`);
                  } else if (tableOpType === "pivot" && tableOpGroupCol && tableOpKey && tableOpAggCol) {
                    const pvs = [...new Set(t.rows.map((r) => String(r[tableOpKey] ?? "")))]; const grps = new Map<string, Record<string, CellValue>>();
                    for (const r of t.rows) { const rk = String(r[tableOpGroupCol] ?? ""); const ck = String(r[tableOpKey] ?? ""); if (!grps.has(rk)) grps.set(rk, { [tableOpGroupCol]: rk }); const g = grps.get(rk)!; g[ck] = typeof r[tableOpAggCol] === "number" ? ((g[ck] as number || 0) + (r[tableOpAggCol] as number)) : r[tableOpAggCol]; }
                    const res: TableData = { id: crypto.randomUUID(), name: `${t.name}_pivot`, columns: [{ name: tableOpGroupCol, dtype: "string", nullRatio: 0, uniqueCount: 0, semanticLabel: tableOpGroupCol, sampleValues: [] }, ...pvs.map((v) => ({ name: v, dtype: "float" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: "measure", sampleValues: [] }))], rows: [...grps.values()] };
                    setTableOpResult(res); addTable(res); addLog(`Pivot: rows=${tableOpGroupCol} cols=${tableOpKey}`);
                  } else if (tableOpType === "union") { const t2 = tables.find((tb) => tb.id === tableOpT2); if (!t2) return; const all = [...new Set([...cols, ...t2.columns.map((c) => c.name)])];
                    const rows = [...t.rows, ...t2.rows].map((r) => { const nr: Record<string, CellValue> = {}; all.forEach((c) => (nr[c] = r[c] ?? null)); return nr; });
                    const res: TableData = { id: crypto.randomUUID(), name: `union`, columns: all.map((c) => ({ name: c, dtype: "string" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: c, sampleValues: [] })), rows };
                    setTableOpResult(res); addTable(res); addLog(`Union: ${rows.length} صف`);
                  } else if (tableOpType === "join") { const t2 = tables.find((tb) => tb.id === tableOpT2); if (!t2 || !tableOpKey || !tableOpKey2) return;
                    const m = new Map<string, Record<string, CellValue>[]>(); for (const r of t2.rows) { const k = String(r[tableOpKey2] ?? ""); if (!m.has(k)) m.set(k, []); m.get(k)!.push(r); }
                    const joined: Record<string, CellValue>[] = []; const t2c = t2.columns.map((c) => c.name).filter((c) => c !== tableOpKey2);
                    for (const r of t.rows) { const k = String(r[tableOpKey] ?? ""); const ms = m.get(k) || []; if (ms.length > 0) { for (const mr of ms) { const nr = { ...r }; t2c.forEach((c) => (nr[`${t2.name}.${c}`] = mr[c])); joined.push(nr); } } else if (tableOpJoinType !== "inner") { const nr = { ...r }; t2c.forEach((c) => (nr[`${t2.name}.${c}`] = null)); joined.push(nr); } }
                    const res: TableData = { id: crypto.randomUUID(), name: `${tableOpJoinType}_join`, columns: [...t.columns, ...t2c.map((c) => ({ name: `${t2.name}.${c}`, dtype: "string" as ColumnDType, nullRatio: 0, uniqueCount: 0, semanticLabel: c, sampleValues: [] }))], rows: joined };
                    setTableOpResult(res); addTable(res); addLog(`${tableOpJoinType} Join: ${joined.length} صف`);
                  } else if (tableOpType === "split" && tableOpGroupCol) {
                    const grps = new Map<string, Record<string, CellValue>[]>(); for (const r of t.rows) { const k = String(r[tableOpGroupCol] ?? "other"); if (!grps.has(k)) grps.set(k, []); grps.get(k)!.push(r); }
                    grps.forEach((rows, key) => addTable({ id: crypto.randomUUID(), name: `${t.name}_${key}`, columns: t.columns, rows })); addLog(`Split: ${grps.size} جداول`);
                  }
                };
                return (
                  <div className="space-y-4">
                    <div className="flex gap-2 flex-wrap">{(["join", "union", "group", "pivot", "split"] as const).map((op) => (<button key={op} onClick={() => setTableOpType(op)} className={`px-3 py-1.5 rounded-lg text-sm ${tableOpType === op ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-800"}`}>{{ join: "Join", union: "Union", group: "Group", pivot: "Pivot", split: "Split" }[op]}</button>))}</div>
                    <div className="flex gap-2 flex-wrap items-end">
                      {(tableOpType === "join" || tableOpType === "union") && <div><label className="block text-xs text-gray-500 mb-1">جدول 2</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpT2} onChange={(e) => setTableOpT2(e.target.value)}><option value="">اختر</option>{others.map((tb) => <option key={tb.id} value={tb.id}>{tb.name}</option>)}</select></div>}
                      {tableOpType === "join" && <><div><label className="block text-xs text-gray-500 mb-1">نوع</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpJoinType} onChange={(e) => setTableOpJoinType(e.target.value as any)}><option value="inner">Inner</option><option value="left">Left</option><option value="right">Right</option><option value="full">Full</option></select></div><div><label className="block text-xs text-gray-500 mb-1">مفتاح 1</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpKey} onChange={(e) => setTableOpKey(e.target.value)}><option value="">اختر</option>{cols.map((c) => <option key={c} value={c}>{c}</option>)}</select></div><div><label className="block text-xs text-gray-500 mb-1">مفتاح 2</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpKey2} onChange={(e) => setTableOpKey2(e.target.value)}><option value="">اختر</option>{(tables.find((tb) => tb.id === tableOpT2)?.columns || []).map((c) => <option key={c.name} value={c.name}>{c.name}</option>)}</select></div></>}
                      {(tableOpType === "group" || tableOpType === "pivot" || tableOpType === "split") && <div><label className="block text-xs text-gray-500 mb-1">عمود التجميع</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpGroupCol} onChange={(e) => setTableOpGroupCol(e.target.value)}><option value="">اختر</option>{cols.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>}
                      {tableOpType === "pivot" && <div><label className="block text-xs text-gray-500 mb-1">عمود المحور</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpKey} onChange={(e) => setTableOpKey(e.target.value)}><option value="">اختر</option>{cols.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>}
                      {(tableOpType === "group" || tableOpType === "pivot") && <><div><label className="block text-xs text-gray-500 mb-1">عمود القيم</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpAggCol} onChange={(e) => setTableOpAggCol(e.target.value)}><option value="">اختر</option>{cols.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>{tableOpType === "group" && <div><label className="block text-xs text-gray-500 mb-1">دالة</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tableOpAgg} onChange={(e) => setTableOpAgg(e.target.value as any)}><option value="sum">مجموع</option><option value="avg">متوسط</option><option value="count">عدد</option><option value="min">أدنى</option><option value="max">أعلى</option></select></div>}</>}
                    ...
                    {tableOpResult && <DataTable data={tableOpResult} maxRows={10} />}
                  </div>
                );
              })()}
            </SectionCard>
          ...
          {/* === SECTION 8: T-IR === */}
          {activeSection === "tir" && (
            <SectionCard title="التحويلات (Transform IR)" icon="transform" defaultOpen>
              <div className="space-y-4">
                <div className="flex gap-2 flex-wrap items-end">
                  <div><label className="block text-xs text-gray-500 mb-1">نوع</label><select className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800" value={tirType} onChange={(e) => setTirType(e.target.value as TIRBlockType)}>{(["select", "rename", "filter", "sort", "cast", "derive", "join", "group", "pivot"] as TIRBlockType[]).map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
                  <div><label className="block text-xs text-gray-500 mb-1">معامل 1</label><input className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 w-40" value={tirP1} onChange={(e) => setTirP1(e.target.value)} /></div>
                  <div><label className="block text-xs text-gray-500 mb-1">معامل 2</label><input className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 w-40" value={tirP2} onChange={(e) => setTirP2(e.target.value)} /></div>
                  {(tirType === "filter" || tirType === "group" || tirType === "join") && <div><label className="block text-xs text-gray-500 mb-1">معامل 3</label><input className="border rounded-lg px-3 py-2 text-sm dark:bg-gray-800 w-40" value={tirP3} onChange={(e) => setTirP3(e.target.value)} /></div>}
                  <button className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700" onClick={() => {
                    const params: Record<string, unknown> = {};
                    if (tirType === "select") params.columns = tirP1.split(",").map((s) => s.trim()); else if (tirType === "rename") { params.from = tirP1; params.to = tirP2; } else if (tirType === "filter") { params.column = tirP1; params.op = tirP2 || ">"; params.value = tirP3; } else if (tirType === "sort") { params.column = tirP1; params.direction = tirP2 || "asc"; } else if (tirType === "cast") { params.column = tirP1; params.targetType = tirP2 || "text"; } else if (tirType === "derive") { params.name = tirP1; params.expression = tirP2; } else if (tirType === "group") { params.keys = [tirP1]; params.agg = tirP2; params.column = tirP3; } else if (tirType === "pivot") { params.pivotColumn = tirP1; params.valueColumn = tirP2; } else if (tirType === "join") { params.leftKey = tirP1; params.rightKey = tirP2; params.joinType = tirP3 || "inner"; }
                    setTirBlocks((p) => [...p, { id: crypto.randomUUID(), type: tirType, label: tirType, params }]); addLog(`T-IR: +${tirType}`);
                  }}><MIcon name="add" className="text-lg align-middle ml-1" /> إضافة</button>
                </div>
                {tirBlocks.length > 0 && <div className="space-y-2">
                  {tirBlocks.map((b, i) => (<div key={b.id} className="flex items-center gap-2 p-3 border rounded-lg bg-gray-50 dark:bg-gray-800"><span className="bg-blue-100 text-blue-700 rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold">{i + 1}</span><LocalBadge color="purple">{b.type}</LocalBadge><span className="text-sm flex-1">{JSON.stringify(b.params)}</span><button onClick={() => setTirBlocks((p) => p.filter((x) => x.id !== b.id))} className="text-red-400 hover:text-red-600"><MIcon name="delete" className="text-lg" /></button></div>))}
                  <div className="flex gap-2"><button className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm" onClick={() => { navigator.clipboard.writeText(JSON.stringify(tirBlocks, null, 2)); addLog("T-IR JSON copied"); }}><MIcon name="content_copy" className="text-lg align-middle ml-1" /> نسخ JSON</button><button className="bg-red-500 text-white px-4 py-2 rounded-lg text-sm" onClick={() => setTirBlocks([])}><MIcon name="delete_sweep" className="text-lg align-middle ml-1" /> مسح</button></div>
                </div>}
              </div>
            </SectionCard>
          )}

          {/* === SECTION 9: Power Query M === */}
          {activeSection === "powerQuery" && (
            <SectionCard title="تصدير Power Query M" icon="code" defaultOpen>
              {tirBlocks.length === 0 ? <p className="text-gray-400">أضف كتل T-IR أولاً</p> : (() => {
                const mCode = tirToM(tirBlocks, activeTable?.name || "Table1");
                return (<div className="space-y-3"><div className="flex items-center justify-between"><h4 className="font-medium text-sm">Power Query M:</h4><button className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs" onClick={() => { navigator.clipboard.writeText(mCode); addLog("M code copied"); }}><MIcon name="content_copy" className="text-sm align-middle ml-1" /> نسخ</button></div><pre className="bg-gray-900 text-green-400 p-4 rounded-lg text-sm overflow-x-auto font-mono leading-relaxed" dir="ltr">{mCode}</pre></div>);
              })()}
            </SectionCard>
          )}
