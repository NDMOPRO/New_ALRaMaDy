...
exports.registerTranscriptionCapability = exports.dispatchTranscriptionTool = exports.dispatchTranscriptionAction = exports.TranscriptionExtractionEngine = exports.defaultTranscriptionStorageRoot = exports.TranscriptionExtractionStore = void 0;
...
Object.defineProperty(exports, "TranscriptionExtractionStore", { enumerable: true, get: function () { return store_2.TranscriptionExtractionStore; } });
Object.defineProperty(exports, "defaultTranscriptionStorageRoot", { enumerable: true, get: function () { return store_2.defaultTranscriptionStorageRoot; } });
const CompareRequestSchema = zod_1.z.object({
    left_bundle_ref: zod_1.z.string(),
    right_bundle_ref: zod_1.z.string(),
    actor_ref: zod_1.z.string(),
    workspace_id: zod_1.z.string(),
    tenant_ref: zod_1.z.string(),
    storage_dir: zod_1.z.string().optional()
});
const TranscriptionDispatchRequestSchema = zod_1.z.object({
    action_id: zod_1.z.string(),
    payload: zod_1.z.unknown(),
    storage_dir: zod_1.z.string().optional()
});
const TranscriptionToolDispatchRequestSchema = zod_1.z.object({
...
const detectInputKind = (fileName) => {
    const extension = node_path_1.default.extname(fileName).toLowerCase();
    ...
    if ([".mp4", ".mov", ".avi", ".webm", ".mkv"].includes(extension))
        return "video_file";
    if (extension === ".pdf")
        return lowerName.includes("scan") ? "scanned_document" : "pdf";
    if ([".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".webp"].includes(extension)) {
        if (lowerName.includes("table") || lowerName.includes("grid") || lowerName.includes("sheet"))
            return "image_table";
        return "image_text";
    }
...
const normalizeText = (value) => value
    ...
    .replace(/\n{3,}/g, "\n\n")
    .trim();
const detectLanguage = (value) => (/[\u0600-\u06FF]/.test(value) ? "ar" : "en");
const sentenceScore = (sentence) => {
    let score = 0;
    if (/\d/.test(sentence))
        score += 2;
    if (/(must|should|need|required|action|deadline|meeting|risk|issue|invoice|total|summary|يجب|مطلوب|إجراء)/i.test(sentence))
        score += 3;
...
const resolvePythonExecutable = () => {
    const candidate = node_path_1.default.resolve(process.cwd(), "..", ".venv311-strict", "Scripts", "python.exe");
    if (node_fs_1.default.existsSync(candidate)) {
        return candidate;
    }
    return "python";
};
const bridgeScriptPath = () => node_path_1.default.resolve(process.cwd(), "packages", "transcription-extraction-engine", "tools", "content_bridge.py");
const bridgeRequestPath = (root, name) => node_path_1.default.join(root, `${name}.request.json`);
const bridgeOutputPath = (root, name) => node_path_1.default.join(root, `${name}.output.json`);
const namedValueMatches = (text) => {
    const matches = [];
    const regex = /^([^\n:]{2,60}):\s*(.+)$/gm;
    let match;
...
const extractAmounts = (text) => [...new Set([...text.matchAll(/(?:SAR|USD|EUR|\$|€|£)?\s?\d[\d,]*(?:\.\d+)?/g)].map((match) => match[0].trim()).filter((value) => /\d/.test(value)))];
const extractProperEntities = (text) => [
    ...new Set([...text.matchAll(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2}\b/g)]
        .map((match) => match[0].trim())
        .filter((value) => value.length > 2))
];
const inferClassification = (inputKind, text, tables) => {
    if (inputKind === "audio_file" || inputKind === "video_file")
        return "meeting_transcript";
    if (/invoice|amount due|total/i.test(text))
        return "invoice";
    if (/meeting|agenda|attendees/i.test(text))
        return "meeting_notes";
    if (tables > 0)
        return "tabular_document";
    if (/risk|issue|action/i.test(text))
        return "operations_report";
    return "general_document";
};
const pickTopSentences = (sentences, limit) => [...sentences]
    .sort((left, right) => sentenceScore(right) - sentenceScore(left))
    .slice(0, limit)
    .filter((sentence, index, array) => array.indexOf(sentence) === index);
const summarize = (title, text) => {
    ...
    return [
        { schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE, schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION, summary_id: id("summary", title, "brief"), source_refs: [], level: "brief", title: `${title} brief`, summary_text: briefSentences.join(" "), bullet_points: briefSentences, evidence_refs: [] },
        { schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE, schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION, summary_id: id("summary", title, "standard"), source_refs: [], level: "standard", title: `${title} summary`, summary_text: standardSentences.join(" "), bullet_points: standardSentences, evidence_refs: [] },
        { schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE, schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION, summary_id: id("summary", title, "detailed"), source_refs: [], level: "detailed", title: `${title} detailed`, summary_text: detailedSentences.join(" "), bullet_points: detailedSentences, evidence_refs: [] },
        { schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE, schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION, summary_id: id("summary", title, "executive"), source_refs: [], level: "executive", title: `${title} executive`, summary_text: standardSentences.slice(0, 3).join(" "), bullet_points: standardSentences.slice(0, 5), evidence_refs: [] }
    ];
};
const tokenise = (value) => normalizeText(value)
    .toLowerCase()
    .split(/[^a-z0-9\u0600-\u06FF]+/i)
    .filter((token) => token.length > 1);
const makeStorageRef = (prefix, objectId, region) => ({
...
const createAuditEvent = (tenantRef, workspaceId, actorRef, actionRef, jobRef, objectRefs, metadata) => contracts_1.AuditEventSchema.parse({
...
const readBridgeResponse = (filePath) => JSON.parse(node_fs_1.default.readFileSync(filePath, "utf8"));
const buildCanonical = (bundle) => {
...
const buildQueryDataset = (bundle) => ({
...
const buildReportHandoff = (bundle) => ({
...
const buildQuestionAnswer = (bundle, question) => {
    ...
    const matchedSegments = bundle.segments
        ...
        .slice(0, 5);
    const matchedFields = bundle.fields.filter((field) => questionTokens.some((token) => tokenise(`${field.field_name} ${field.normalized_value}`).includes(token)));
    const matchedEntities = bundle.entities.filter((entity) => questionTokens.some((token) => tokenise(entity.normalized_value).includes(token)));
    ...
    if (compareIntent && bundle.sources.length > 1) {
        ...
        const leftSource = bundle.sources.find((source) => lowerQuestion.includes("pdf") && source.input_kind === "pdf") ??
            bundle.sources.find((source) => lowerQuestion.includes("audio") && source.input_kind === "audio_file") ??
            bundle.sources[0];
        const rightSource = bundle.sources.find((source) => lowerQuestion.includes("excel") && source.input_kind === "spreadsheet_file") ??
    ...
    return contracts_1.TranscriptionQuestionAnswerSchema.parse({
        schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
        schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
...
class TranscriptionExtractionEngine {
    constructor(options) {
        this.store = new store_1.TranscriptionExtractionStore(options?.storageDir);
        this.pythonExecutable = options?.pythonExecutable ?? resolvePythonExecutable();
    }
    callBridge(inputPath, inputKind, workDir) {
        const requestPath = bridgeRequestPath(workDir, node_path_1.default.basename(inputPath));
        const outputPath = bridgeOutputPath(workDir, node_path_1.default.basename(inputPath));
        node_fs_1.default.mkdirSync(workDir, { recursive: true });
        node_fs_1.default.writeFileSync(requestPath, JSON.stringify({ input_path: inputPath, input_kind: inputKind, work_dir: workDir }, null, 2), "utf8");
        const result = (0, node_child_process_1.spawnSync)(this.pythonExecutable, [bridgeScriptPath(), "analyze", "--request", requestPath, "--output", outputPath], {
            encoding: "utf8"
        });
        if (result.status !== 0) {
    ...
    async analyzeSpreadsheet(inputPath) {
        ...
        workbook.worksheets.forEach((worksheet, sheetIndex) => {
            const rows = worksheet.getSheetValues().slice(1).map((row) => Array.isArray(row)
                ? row
                    .slice(1)
        ...
        return {
            status: "success",
            file_name: node_path_1.default.basename(inputPath),
    ...
    async ingestAndExtract(input) {
        const request = contracts_1.TranscriptionJobRequestSchema.parse({
            ...input,
        ...
        const globalAlignedWords = [];
        const globalDisagreements = [];
        const globalOnScreenText = [];
        const globalTables = [];
        const globalFields = [];
        ...
        const semanticNodes = [];
        const semanticEdges = [];
        const jobRoot = node_path_1.default.join(this.store.rootDir, "jobs", jobId, "bridge");
        const auditEvents = [];
        ...
        for (const entry of attachmentPaths) {
            ...
            const response = inputKind === "spreadsheet_file" && [".xlsx", ".xlsm", ".csv"].includes(node_path_1.default.extname(entry.targetPath).toLowerCase())
                ? await this.analyzeSpreadsheet(entry.targetPath)
                : this.callBridge(entry.targetPath, inputKind, node_path_1.default.join(jobRoot, sourceId));
            ...
            const source = {
                ...
                file_size_bytes: node_fs_1.default.statSync(entry.targetPath).size,
            ...
            for (const speaker of response.speakers ?? []) {
                ...
                globalSpeakers.push({
                    ...
                    detection_method: speaker.detection_method ?? "heuristic",
            ...
            for (const section of response.sections ?? []) {
                ...
                globalSections.push({
                    ...
                    section_kind: section.section_kind ?? "body",
            ...
            for (const segment of response.segments ?? []) {
                ...
                const parsed = {
                    ...
                    source_ref: sourceRef,
                    segment_kind: segment.segment_kind ?? "paragraph",
            ...
            for (const word of response.word_timestamps ?? []) {
                ...
                const segmentRef = globalSegments.find((segment) => segment.source_ref === sourceRef &&
                    segment.start_ms !== null &&
                    segment.end_ms !== null &&
                    startMs >= segment.start_ms &&
                    endMs <= segment.end_ms)?.segment_id ?? null;
            ...
            for (const disagreement of response.disagreements ?? []) {
                ...
                globalDisagreements.push({
                    ...
                    resolution_status: disagreement.resolution_status ?? "open",
                    evidence_ref: evidenceRef
                });
                evidenceRefs.push(evidenceRef);
            }
            ...
            for (const table of response.tables ?? []) {
                ...
                const parsed = {
                    ...
                    headers: (table.headers ?? []).map((header) => normalizeText(String(header))),
                    rows: (table.rows ?? []).map((row) => row.map((cell) => normalizeText(String(cell)))),
                    bbox: table.bbox ?? null,
                    extraction_method: table.extraction_method ?? "ocr_table",
            ...
            const effectiveText = sourceText || globalSegments.filter((segment) => segment.source_ref === sourceRef).map((segment) => segment.text).join("\n");
            ...
            dates.forEach((value) => globalEntities.push({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            amounts.forEach((value) => globalEntities.push({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            properEntities.forEach((value) => globalEntities.push({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            const actionSegments = globalSegments.filter((segment) => segment.source_ref === sourceRef && /(must|should|need|required|action|يجب|مطلوب|يلزم)/i.test(segment.text));
            actionSegments.forEach((segment, index) => globalActionItems.push({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            semanticEdges.push(...sourceFieldIds.map((fieldId) => ({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            auditEvents.push(createAuditEvent(request.tenant_ref, request.workspace_id, request.created_by, "transcription.source_ingested.v1", jobId, [sourceRef], {
                stage: "parsing",
        ...
        const fieldMap = globalFields.reduce((accumulator, field) => {
            var _a;
            accumulator[_a = field.field_name] ?? (accumulator[_a] = []);
        ...
        const multimodalCorroborated = mediaSourceCount === 0 ||
        ...
        const verificationPenalty = unresolvedDisagreementRefs.length * 0.18 +
        ...
        const verificationGate = {
            ...
            exact: unresolvedDisagreementRefs.length === 0 &&
            ...
            summary: unresolvedDisagreementRefs.length > 0
        ...
        qaSeeds.forEach((seed) => {
            ...
            seed.supporting_segment_refs.forEach((segmentRef) => semanticEdges.push({
                schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
                schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
        ...
        summaries.forEach((summary) => semanticNodes.push({
            schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
            schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
        ...
        const bundle = contracts_1.UnifiedContentBundleSchema.parse({
            contract: contracts_1.TRANSCRIPTION_CONTRACT,
            schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
            schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            bundle_id: bundleId,
            capability_id: contracts_1.TRANSCRIPTION_CAPABILITY_ID,
        ...
        const job = contracts_1.JobSchema.parse({
            ...
            job_id: jobId,
            capability: contracts_1.TRANSCRIPTION_CAPABILITY_ID,
        ...
        auditEvents.push(createAuditEvent(request.tenant_ref, request.workspace_id, request.created_by, "transcription.extraction_generated.v1", jobId, [bundleId], { stage: "executing", fields: globalFields.length, entities: globalEntities.length, tables: globalTables.length }), createAuditEvent(request.tenant_ref, request.workspace_id, request.created_by, "transcription.exactness_gate.v1", jobId, [bundleId, verificationGate.gate_id], { stage: "verification", exact: verificationGate.exact, verification_score: verificationGate.verification_score, unresolved_disagreements: verificationGate.unresolved_disagreement_refs.length }), createAuditEvent(request.tenant_ref, request.workspace_id, request.created_by, "transcription.job_completed.v1", jobId, [bundleId, evidencePack.evidence_pack_id], { stage: degradedReasons.length > 0 ? "degraded" : "completed", verification_status: verificationStatus }));
        const lineageEdges = [
        ...
        const libraryAsset = contracts_1.LibraryAssetSchema.parse({
    ...
    compareBundles(input) {
        ...
        const compareResult = contracts_1.TranscriptionCompareResultSchema.parse({
            schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
            schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
        ...
        const diffArtifact = contracts_1.ArtifactSchema.parse({
            contract: (0, contracts_1.contractEnvelope)("artifact"),
    ...
    answerQuestion(input) {
        const question = contracts_1.TranscriptionQuestionRequestSchema.parse(input);
        const bundles = question.bundle_refs.map((bundleRef) => this.store.loadBundle(bundleRef));
        const mergedBundle = bundles.reduce((accumulator, bundle) => contracts_1.UnifiedContentBundleSchema.parse({
            ...accumulator,
            ...
            updated_at: now()
        }), contracts_1.UnifiedContentBundleSchema.parse({
            contract: contracts_1.TRANSCRIPTION_CONTRACT,
            schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE,
            schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION,
            ...
            evidence_refs: [],
            lineage_refs: [],
            language_profile: { schema_namespace: contracts_1.TRANSCRIPTION_SCHEMA_NAMESPACE, schema_version: contracts_1.TRANSCRIPTION_SCHEMA_VERSION, primary_language: "en", detected_languages: ["en"], rtl: false, language_confidence: 0.5 },
            relations: [],
...
exports.TranscriptionExtractionEngine = TranscriptionExtractionEngine;
const transcriptionToolToActionMap = {
...
const dispatchTranscriptionAction = async (input) => {
    ...
    switch (request.action_id) {
        case "transcription.ingest_and_extract.v1":
            return engine.ingestAndExtract(request.payload);
        case "transcription.compare.v1":
            return engine.compareBundles(request.payload);
        case "transcription.answer_question.v1":
            return engine.answerQuestion(request.payload);
        default:
            throw new Error(`Unsupported transcription action: ${request.action_id}`);
    }
};
exports.dispatchTranscriptionAction = dispatchTranscriptionAction;
const dispatchTranscriptionTool = async (input) => {
    ...
    return (0, exports.dispatchTranscriptionAction)({
        action_id: actionId,
        payload: request.payload,
        storage_dir: request.storage_dir
    });
};
exports.dispatchTranscriptionTool = dispatchTranscriptionTool;
const registerTranscriptionCapability = (runtime) => {
    const actions = contracts_1.ActionRegistry.filter((action) => action.capability === contracts_1.TRANSCRIPTION_CAPABILITY_ID);
    const tools = contracts_1.ToolRegistry.filter((tool) => tool.owner_capability === contracts_1.TRANSCRIPTION_CAPABILITY_ID);
    ...
    runtime.registerManifest((0, capability_registry_1.createActionManifest)(contracts_1.TRANSCRIPTION_CAPABILITY_ID, "1.0.0", actions, ["approval.transcription"], ["evidence.transcription"]));
    ...
    runtime.registerEvidenceHook("evidence.transcription", async (pack) => contracts_1.EvidencePackSchema.parse(pack));
};
exports.registerTranscriptionCapability = registerTranscriptionCapability;
